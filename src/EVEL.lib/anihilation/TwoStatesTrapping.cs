...
namespace Evel.engine.anh.stdmodels {
    public class TwoStatesTrapping : Mexm {

        //private double[] conversion = new double[4];

        private void getConversionParameters(IComponent component, out double intD, out double tauD, out double tauEff) {
            double lambdaA;
            double lambdaBulk;
            double kappa;
            //tauD
            tauD = Math.Abs(component[1].Value);
        ...
        protected override double CalculateAverageLifetime(IComponent component, IParameter parameter) {
            if (parameter == component[0] && component.Parent is IGroup) {
                double intD, tauD, tauEff;
                getConversionParameters(((IGroup)component.Parent).Components[0], out intD, out tauD, out tauEff);
                return tauD * intD + (1 - intD) * tauEff;
            } else
                return 0.0;
        }   
        ...
        public override string Name {
            get { return "Two states trapping model"; }
        }
        ...
        protected override GroupDefinition[] getGroupsDefinition() {
            if (this._groupsDefinition == null) {
                ...
                this._groupsDefinition[1].parameters = new ParameterDefinition[] { 
                    ...
                    new ParameterDefinition("tauD", "[p text='t' font='symbol'][p text='D' index='sub']", ParameterStatus.Common | ParameterStatus.Free, 0),//ParameterProperties.IsDependency), 
                    new ParameterDefinition("kappaD", "[p text='k' font='symbol'][p text='D' index='sub']", ParameterStatus.Local | ParameterStatus.Free, 0),//ParameterProperties.IsDependency),
