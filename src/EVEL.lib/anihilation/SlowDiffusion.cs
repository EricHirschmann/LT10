...
namespace Evel.engine.anh.stdmodels
{
    public class SlowDiffusion : TwoStatesTrapping
    {
        ...
        private void AppendLtParams(double weight, double kapc, double lamPs, double lam_diff,
            double sig, double taumin, Func<LTCurveParams> getCurrentParams, LTCurveParams diffsParameter)
        {

            if (sig > 0.0)
            {
                double norm = 1 / Math.Sqrt(2 * Math.PI) / sig;
                ...
                Action<double, double> ff = (double u, double _c) =>
                {
                    if (Math.Abs(u) < 80)
                    {
                        double lambda = 1 / (Math.Exp(u) + taumin);
                        double c = weight * kapc / (lam_diff - lambda) * norm * Math.Exp(-(u - uu) * (u - uu) * mian);
            ...
            {
                double tmpc = weight * kapc / (lam_diff - lamPs);
                LTCurveParams ltp = getCurrentParams();
                ltp.tau = 1 / lam_diff;
                ltp.fraction = tmpc;
                diffsParameter.fraction -= tmpc * lamPs / lam_diff;
            }
        }
        ...
        protected override GroupDefinition[] getGroupsDefinition()
        {
            if (this._groupsDefinition == null)
            {
                ...
                this._groupsDefinition[1].parameters = new ParameterDefinition[] { 
                    ...
                    new ParameterDefinition("kappa1", "[p text='k' font='symbol'][p text='2' index='sub']", ParameterStatus.Local | ParameterStatus.Free, 0),
        ...
        public override void convert(List<ICurveParameters> curveParameters, IParameterSet parameters)
        {
            ...
            double lambdaortho1 = 1 / parameters[1].Components[0]["tauPo1"].Value + 1.0 / 142.0;
            double lampara1 = 1 / parameters[1].Components[0]["tauPo1"].Value + 1.0 / 0.124;
            double lambdaortho2 = 1 / parameters[1].Components[0]["tauPo2"].Value + 1.0 / 142.0;
            double lampara2 = 1 / parameters[1].Components[0]["tauPo2"].Value + 1.0 / 0.124;
            double lambdaTrapp = 1 / parameters[1].Components[0]["tautrapp"].Value;
            double lam = 1 / parameters[1].Components[0]["taufree"].Value + kappa1 + kappa2 + mi;
            ...
            {
                //ortho-Ps
                AppendLtParams(0.75, kappa1, lambdaortho1, lam, sigma1, 0.54, GetCurrentParameters, diffsParameters);

                //para-Ps
                AppendLtParams(0.25, kappa1, lampara1, lam, 0.0, 0.54, GetCurrentParameters, diffsParameters);
            }

            if (kappa2 > eps)
            {
                //ortho-Ps
                AppendLtParams(0.75, kappa2, lambdaortho2, lam, sigma1, 0.54, GetCurrentParameters, diffsParameters);

                //para-Ps
                AppendLtParams(0.25, kappa2, lampara2, lam, 0.0, 0.54, GetCurrentParameters, diffsParameters);
            }

            if (mi > eps)
            {
                AppendLtParams(1, mi, lambdaTrapp, lam, sigmaTrapp, 0.0, GetCurrentParameters, diffsParameters);
            }
            ...
            //bulk
            {
                ...
                pfree.tau = lam;
                pfree.fraction = 1 / Math.Abs(parameters[1].Components[0]["taufree"].Value) / lam;
            }

            diffsParameters.tau = lam;
            ...
            System.Diagnostics.Debug.Assert(Math.Abs(sum - 1.0) < 1e-5);
#endif
